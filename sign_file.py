...
def authenticode_sign_binary(token, inputfile, outputfile):
    try:
        ...
        os.system(
            "openssl req -x509 -new -keyout {tmpdir}/rootCA.key -out {tmpdir}/rootCA.crt -config {tmpdir}/root-ca.conf -days 365 -nodes".format(
                tmpdir=tmpdir
            )
        )
        os.system(
            'openssl req -new -keyout {tmpdir}/cert.key -out {tmpdir}/cert.csr -nodes -subj "/C=US/ST=Washington/L=Redmond/O=Microsoft Corporation/CN=Microsoft Windows"'.format(
                tmpdir=tmpdir
            )
        )
        os.system(
            "openssl ca -batch -config {tmpdir}/root-ca.conf -cert {tmpdir}/rootCA.crt -keyfile {tmpdir}/rootCA.key -in {tmpdir}/cert.csr -out {tmpdir}/cert.crt".format(
                tmpdir=tmpdir,
            )
        )
        os.system(
            "osslsigncode sign -certs {tmpdir}/cert.crt -key {tmpdir}/cert.key -in {inputfile} -out {outputfile}".format(
...
def main(argv):
    ...
    try:
        opts, args = getopt.getopt(
            argv, "ht:f:o:", ["token=", "inputfile=", "outputfile="]
        )
