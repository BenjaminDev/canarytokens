...
from queries import (
    create_bitcoin_account, create_linkedin_account,
...
with open('/workspace/templates/error_http.html', 'r') as f:
...
class GeneratorPage(resource.Resource):
    ...
    def render_GET(self, request):
        ...
        return template.render(settings=settings, sites_len=sites_len, now=now).encode(
            'utf8',
        )
        ...
        try:
            ...
            try:
                if not request.args.get('type', None)[0] == 'aws_keys':
                    raise Exception()
                keys = get_aws_keys(
                    token=canarytoken.value(), server=get_all_canary_domains()[0],
                )
                if not keys:
                    response['Error'] = 4
                    response[
                        'Error_Message'
                    ] = 'Failed to retrieve AWS API keys. Please contact support@thinkst.com.'
                    raise Exception()
            ...
            try:
                ...
                if kubeconfig is None:
                    ...
                    response[
                        'Error_Message'
                    ] = 'Failed to retrieve the kubeconfig. Please contact support@thinkst.com.'
                    raise Exception()
            ...
            try:
                ...
                if len(filebody) > int(settings.MAX_UPLOAD_SIZE):
                    response['Error'] = 4
                    response['Message'] = (
                        'File too large. File size must be < '
                        + str(int(settings.MAX_UPLOAD_SIZE) / (1024 * 1024))
                        + 'MB.'
                    )
                ...
                filepath = (
                    os.path.join(settings.WEB_IMAGE_UPLOAD_PATH, r[:2], r[2:]) + ext
                )
            ...
            try:
                ...
                signed_contents = make_canary_authenticode_binary(
                    hostname=canarydrop.get_hostname(with_random=False, as_url=True),
                    filebody=filebody,
                )
                response['file_name'] = filename
                response[
                    'file_contents'
                ] = 'data:octet/stream;base64,' + base64.b64encode(signed_contents)
            except:
                pass
            ...
            if token_type == 'wireguard':
                canarydrop['wg_key'] = wg.generateCanarytokenPrivateKey(
    ...
    def render_GET(self, request):
        try:
            ...
            if fmt == 'zip':
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={token}.zip'.format(token=token),
                )
                return make_canary_zip(
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={token}.docx'.format(token=token),
                )
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={token}.xlsx'.format(token=token),
                )
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={token}.pdf'.format(token=token),
                )
                return make_canary_pdf(
                    hostname=canarydrop.get_hostname(nxdomain=True, with_random=False),
                )
            elif fmt == 'awskeys':
                ...
                text = '[default]\naws_access_key={id}\naws_secret_access_key={k}\nregion={r}\noutput={o}'.format(
                    id=canarydrop['aws_access_key_id'],
                    k=canarydrop['aws_secret_access_key'],
                    r=canarydrop['region'],
                    o=canarydrop['output'],
                )
                return text
                ...
                request.setHeader('Content-Type', 'text/plain')
                request.setHeader(
                    'Content-Disposition', 'attachment; filename=slack_creds',
                )
                text = '# Slack API key\nslack_api_key = {key}'.format(
                    key=canarydrop['slack_api_key'],
                )
                return text
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={token}_history.json'.format(token=token),
                )
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={token}_history.csv'.format(token=token),
                )
                csvOutput = StringIO()
                ...
                for key in incident_list:
                    ...
                    data = [
                        datetime.datetime.fromtimestamp(float(key)).strftime(
                            '%Y-%m-%d %H:%M:%S.%s',
                        ),
                    ] + items
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={token}_mysql_dump.sql.gz'.format(
                        token=token,
                    ),
                )
    ...
    def render_POST(self, request):
        try:
            ...
            if fmt == 'authenticode':
                ...
                request.setHeader(
                    'Content-Disposition',
                    'attachment; filename={filename}.signed'.format(filename=filename),
                )
...
class ManagePage(resource.Resource):
    ...
    def render_GET(self, request):
            ...
            if canarydrop.get('triggered_list', None):
                for timestamp in list(canarydrop['triggered_list'].keys()):
                    formatted_timestamp = datetime.datetime.fromtimestamp(
                        float(timestamp),
                    ).strftime('%Y %b %d %H:%M:%S (UTC)')
                    canarydrop['triggered_list'][formatted_timestamp] = canarydrop[
                        'triggered_list'
                    ].pop(timestamp)
        ...
        return template.render(
            canarydrop=canarydrop, API_KEY=g_api_key, now=now,
        ).encode('utf8')
    ...
    def render_POST(self, request):
        try:
            ...
            try:
                webhook_enable_status = (
                    request.args.get('webhook_enable', None)[0] == 'on'
                )
            except (TypeError, IndexError):
            ...
            template = env.get_template('manage.html')
            return template.render(
                canarydrop=canarydrop, saved=True, settings=settings, API_KEY=g_api_key,
            ).encode('utf8')
            ...
            log.error(
                'Exception in manage.html: {e}, {stack}'.format(
                    e=e, stack=traceback.format_exc(),
                ),
            )
            template = env.get_template('manage.html')
            return template.render(
                canarydrop=canarydrop, error=e, settings=settings,
            ).encode('utf8')
...
class LimitedFile(File):
    def directoryListing(self):
        dl = DirectoryLister(
            self.path, [], self.contentTypes, self.contentEncodings, self.defaultType,
        )
        dl.template = ''
        return dl
...
class CanarytokensHttpd:
    def __init__(self, port=80):
        ...
        # self.service.dataRecieved = lambda x: print(x)
        return None
