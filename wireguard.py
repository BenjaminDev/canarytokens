...
#         print(
#             "Console public key: ",
#             device.privateKey.public_key.encode(encoder=nacl.encoding.Base64Encoder),
#         )
...
#             mac1 = blake2s(
#                 data[:smac1], digest_size=BLAKE2S_128_SIZE, key=deviceCandidate.mac1_key
#             ).digest()
...
#         if not canarytoken:
#             log.debug(
#                 "No matching token for valid handshake with client key {} from {}. Expected when Canarytoken is deleted, but WG client config still in use.".format(
#                     public_key, src
#                 )
#             )
#             return
...
#     wg_key = "{}|{}".format(
#         device_key_idx, privateKey.encode(encoder=nacl.encoding.Base64Encoder)
#     )
#     return wg_key
...
#         private_key, encoder=nacl.encoding.Base64Encoder
#     )
#     public_key = privateKey.public_key.encode(encoder=nacl.encoding.Base64Encoder)
...
#         serverPublicKey=DEVICES[device_key_idx].privateKey.public_key.encode(
#             encoder=nacl.encoding.Base64Encoder
#         ),
...
#         keepAlive=30
#         * (random.randrange(10) + 1),  # Keep Alive *must* be non-zero to trigger token
#     )
