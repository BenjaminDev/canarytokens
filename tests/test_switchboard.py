...
async def test_switchboard_no_channels():
    ...
    with pytest.raises(InvalidChannel):
        await switchboard.dispatch(input_channel='not_valid', canarydrop=None, src_ip='10.2.1.1', src_data={'some': 'data'})
...
@pytest.mark.asyncio(asyncio_mode='strict')
@pytest.mark.parametrize('alert_webhook_url', ['https://hooks.slack.com/test/webhook', 'https://example.com/test'])
async def test_switchboard_register_input_channel(settings, alert_webhook_url):
    ...
    switchboard.add_input_channel(
        name='tester', channel=InputChannel(
            switchboard=switchboard,
            name='tester', settings=settings,
        ),
    )
    ...
    await switchboard.dispatch(input_channel='tester', canarydrop=cd, src_ip='10.2.1.1', src_data={'some': 'data'})
    await switchboard.dispatch(input_channel='tester', canarydrop=cd, src_ip='10.0.1.1', src_data={'more': 'data'})
