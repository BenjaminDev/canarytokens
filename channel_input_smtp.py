...
class CanaryESMTP(smtp.ESMTP):
    ...
    def dispatch(
        ...
        self.factory.dispatch(
            canarydrop=self.canarydrop, src_ip=self.src_ip, mail=self.mail
        )
...
class CanarySMTPFactory(smtp.SMTPFactory, InputChannel):
    ...
    def __init__(self, *a, **kw):
        ...
        InputChannel.__init__(
            self, switchboard=self.switchboard, name=self.CHANNEL, unique_channel=False
        )
...
class ChannelSMTP:
    def __init__(self, port=25, switchboard=None):
        self.service = internet.TCPServer(
            port, CanarySMTPFactory(None, switchboard=switchboard)
        )
