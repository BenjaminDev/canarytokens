...
class ChannelLinkedIn(InputChannel):
    ...
    def __init__(self, min_delay=3600 * 24, switchboard=None):
        ...
        super(ChannelLinkedIn, self).__init__(
            switchboard=switchboard, name=self.CHANNEL
        )
    ...
    def poll(self, linkedin_account=None):
        try:
            ...
            log.error("Could not retrieve linkedin view count: {error}".format(error=e))
            return

        if current_count > linkedin_account["count"]:
            canarydrop = Canarydrop(
                **get_canarydrop(canarytoken=linkedin_account["canarytoken"])
            )
            self.dispatch(
                canarydrop=canarydrop,
                count=current_count,
